...
class CS(object):
    ...
    def loadSU(self):
        ...
        self.ncs = len(np.unique(self.data['sx']))
        self.shots = dict(enumerate(np.unique(self.data['sx'])))

    def loadCS(self, index = 1):
        self.index = index
        #Collecting traces from a specific cdp
        if(index <= self.ncs and index > 0):
            traces = []
            traces_index = []
            for i in range(0, len(self.data)):
                if(self.data['sx'][i] == self.getShotPos()):
    ...
    def getShotPos(self):
        return self.shots[self.index-1]
